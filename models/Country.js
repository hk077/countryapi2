...
class Country {
    ...
    async update(data) {
        const { name, capital, population, languages, fun_fact, map_image_url } = data
        const ec = await db.query("SELECT name FROM country WHERE LOWER(name) = LOWER($1);", [name])
        if (ec.rows.length = 0) {
            throw new Error("Country not in the DB")
        }
        const response = await db.query(
            "UPDATE country SET name = $1, capital = $2, population = $3, languages = $4, fun_fact = $5, map_image_url = $6 WHERE name = $7 RETURNING *;",
            [name, capital, population, languages, fun_fact, map_image_url, this.name]
        )
        return new Country(response.rows[0])
    }
